...
private:
    ...
    void saveSchema(const std::string& db, const std::string& table) {
        ...
        for (const auto& col : t->columns) out << col.name << '\t' << col.type << '\t' << col.not_null << '\t' << col.indexed << '\n';
    }
    ...
    void loadTable(const std::string& db, const std::string& table) {
        ...
        while (std::getline(in, line)) {
            ...
            ColumnDef col;
            std::string nn, idx;
            if (std::getline(ss, col.name, '\t') && std::getline(ss, col.type, '\t') && std::getline(ss, nn, '\t') && std::getline(ss, idx)) {
                col.not_null = nn == "1";
                col.indexed = idx == "1";
